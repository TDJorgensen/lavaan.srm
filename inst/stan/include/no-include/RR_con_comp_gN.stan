...
parameters {
  ...
  cholesky_factor_corr[Kd2] chol_g; // Cholesku factor of group-level correlations
...
transformed parameters {
  ...
  // calculate and/or combine expected values
  {
    ...
    for (k in 1:Kd2) {
      ...
      for (d in 1:Nd) {
        // expected values of round-robin variables, given person-level effects
        ...
        // add means and group effects to expected values
        Mu_d[d, idx1] += Mvec[k] + (S_g[k] * GG[ IDg[d], k]);
        Mu_d[d, idx2] += Mvec[k] + (S_g[k] * GG[ IDg[d], k]);
      }
